...
# รูปแบบราคาคู่: "ค่าย ล320-350/1.9 ย350-390/1.7"  หรือ "ค่าย ล350/390/1.7" (/ คั่นช่วงก็ได้)
# กลุ่ม: 1=camp, 2=hi_min, 3=hi_max(optional), 4=hi_rate, 5=lo_min, 6=lo_max(optional), 7=lo_rate
R_ANN = re.compile(
    r"^\s*([^\s].+?)\s*ล\s*(\d+)(?:\s*[-/]\s*(\d+))?\s*/\s*([\d.]+)\s*ย\s*(\d+)(?:\s*[-/]\s*(\d+))?\s*/\s*([\d.]+)\s*$",
    re.IGNORECASE
)
R_O_ANN = re.compile(
    r"^\s*o\s+(.+?)\s*ล\s*(\d+)(?:\s*[-/]\s*(\d+))?\s*/\s*([\d.]+)\s*ย\s*(\d+)(?:\s*[-/]\s*(\d+))?\s*/\s*([\d.]+)\s*$",
    re.IGNORECASE
)
# รูปแบบราคาเดียว: "ค่าย ล320-350/1.9" หรือ "ค่าย ล350/390/1.9" หรือ "ค่าย ย390/1.7"
# กลุ่ม: 1=camp, 2=side(ล/ย), 3=amount_min, 4=amount_max(optional), 5=rate
R_ANN_SINGLE = re.compile(
    r"^\s*([^\s].+?)\s*([ลย])\s*(\d+)(?:\s*[-/]\s*(\d+))?\s*/\s*([\d.]+)\s*$",
    re.IGNORECASE
)
R_O_ANN_SINGLE = re.compile(
...
def _get_play_help_text():
    return (
        ...
        "หรือไม่ใส่ก็ได้ \n\n"
        "✅ สูงชนะ เต็ม: ได้คืน 100%\n"
        "✅ สูงชนะ -5%: ได้คืน 95%\n"
        "✅ สูงชนะ -15%: ได้คืน 85%\n"
        "✅ สูงชนะ -30%: ได้คืน 70%\n"
        "(เช่นเดียวกับฝั่งต่ำ)\n\n"
        "⛔ เสมอ = หัก 3% ทั้งสองฝั่ง\n"
        "⛔ เสมอสูง = สูงหัก 3% / ต่ำเสียหมด\n"
        "⛔ เสมอต่ำ = ต่ำหัก 3% / สูงเสียหมด\n"
        "⛔ เสมอ2 = ทั้งสองฝั่งเสียหมด\n"
        "⛔ จาว = คืนเต็ม ไม่หัก\n\n"
        "___________________\n\n"
        f"👉 รับสูงสุดต่ำ {fmt(SIDE_CAP['LO'])} ต่อ 1 บั้ง\n"
        f"👉 รับสูงสุดสูง {fmt(SIDE_CAP['HI'])} ต่อ 1 บั้ง\n"
        f"👉 แทงขั้นต่ำ {MIN_BET}-{fmt(MAX_BET)} ต่อ 1 คน\n\n"
        ...
        "ตัวอย่าง:\n"
        "ล/1000 ส/1000 ล1000 ส1000=สูง\n"
        "ย/1000 ต/1000 ย1000 ต1000=ต่ำ\n"
        "(ไม่ต้องเว้นวรรค)\n\n"
        "หมายเหตุ // 💥กรณีออกราคา บั้งไฟ หลังปิด ถือว่า จาวทุกรณี\n"
        "และสนามราคารูด ทางกลุ่มจะไม่เปิดราคา จาวทุกกรณี 💥"
    )
PLAY_HELP_TEXT = None  # ใช้ _get_play_help_text() แทน (เรียกหลัง fmt ถูก define แล้ว)
...
# ====== RESULT DEFINITIONS ======
# ระบบผลใหม่:
# ส1.7  = สูงชนะได้ หัก 30%  (จ่าย 0.70)  / ต่ำเสียยอดเดิมพันทั้งหมด
# ส1.85 = สูงชนะได้ หัก 15%  (จ่าย 0.85)  / ต่ำเสียยอดเดิมพันทั้งหมด
# ส1.95 = สูงชนะได้ หัก 5%   (จ่าย 0.95)  / ต่ำเสียยอดเดิมพันทั้งหมด
# ส2    = สูงชนะได้เต็ม       (จ่าย 1.00)  / ต่ำเสียยอดเดิมพันทั้งหมด
# ต1.7  = ต่ำชนะได้ หัก 30%  (จ่าย 0.70)  / สูงเสียยอดเดิมพันทั้งหมด
# ต1.85 = ต่ำชนะได้ หัก 15%  (จ่าย 0.85)  / สูงเสียยอดเดิมพันทั้งหมด
# ต1.95 = ต่ำชนะได้ หัก 5%   (จ่าย 0.95)  / สูงเสียยอดเดิมพันทั้งหมด
# ต2    = ต่ำชนะได้เต็ม       (จ่าย 1.00)  / สูงเสียยอดเดิมพันทั้งหมด
# เสมอสูง = เสมอสูง หัก 3%   / ต่ำเสียทั้งหมด
# เสมอต่ำ = เสมอต่ำ หัก 3%   / สูงเสียทั้งหมด
# เสมอ    = หัก 3% ทั้ง 2 ฝั่ง
...
RESULT_DEFS = {
    ...
    "เสมอ":    {"label": "เสมอ หัก 3% ทั้ง 2 ฝั่ง",            "special": "DRAW_BOTH_FEE"},
    "เสมอ2":   {"label": "เสมอ2 ทั้ง 2 ฝั่งเสียยอดเดิมพัน",   "special": "DRAW_BOTH_LOSE"},
    # จาว
    "จ":   {"label": "จาว (คืนเต็ม ไม่หัก)", "special": "DRAW_0"},
    "จาว": {"label": "จาว (คืนเต็ม ไม่หัก)", "special": "DRAW_0"},
...
def normalize_result_code(code: str) -> str:
    """
    แปลง code ที่ได้จากคำสั่ง s... ให้เป็น key ใน RESULT_DEFS
    ตัดอักษร s/S นำหน้าออก แล้วตรวจสอบว่า match กับ key ใน RESULT_DEFS
    """
    code = (code or "").strip()
    # ตัด s/S นำหน้าออก (เหลือ ส1.7, ต1.85, เสมอสูง, จ ฯลฯ)
    if code.startswith(("S", "s")):
        code = code[1:].strip()
    # normalize: case-insensitive lookup
    # ลอง match โดยตรงก่อน
    if code in RESULT_DEFS:
        return code
    # ลอง lowercase
    code_lower = code.lower()
    for k in RESULT_DEFS:
...
# ==== ระบบบิล: แทงสวนไม่ได้, แต่ฝั่งเดิมสะสมได้ (ไม่ต้อง X ก่อน) ====
def can_bet(state, uid, side, amount):
    """
    - ห้ามแทงสวน (HI แล้วมาแทง LO หรือกลับกัน)
    - แทงฝั่งเดิมซ้ำได้ เครดิตสะสม แต่ห้ามเกินเครดิตที่มี
    - คืน (ok, reason, "ADD") ถ้าเป็นการเพิ่มยอดเดิม
    """
    if state["phase"] != "OPEN":
    ...
    if existing:
        ...
        if side != existing["side"]:
            return (False, f"❌ ห้ามแทงสวน — คุณมีบิลเดิม: {exist_side_th} {fmt(existing['amount'])}  (พิมพ์ X เพื่อยกเลิกก่อน)", None)
        # ฝั่งเดิม: สะสมได้
        new_total = existing["amount"] + amount
        if new_total > USER_SIDE_CAP[side]:
            side_th = "สูง" if side == "HI" else "ต่ำ"
            return (False, f"ฝั่ง{side_th} ต่อคนเกิน {fmt(USER_SIDE_CAP[side])}", None)
        return (True, "", "ADD")  # ADD = เพิ่มยอดเดิม

    if amount < MIN_BET:
        return (False, f"ขั้นต่ำ {MIN_BET}", None)
    if amount > MAX_BET:
        return (False, f"สูงสุด {MAX_BET}", None)

    remain = user_fund_remain(state, uid)
    if remain < amount:
        return (False, f"ทุนคงเหลือไม่พอ (มี {fmt(remain)})", None)
    ...
    if round_total + amount > ROUND_CAP:
        ...
        return (False, f"❌รับบิลไม่ได้❌: รอบนี้เต็ม {fmt(ROUND_CAP)} - เหลือรับได้อีก {fmt(remain_round)}", None)
...
def flex_open(pair_no, note=None):
    ...
    if note:
        note_rows = [
            {"type": "separator", "color": "#E5E7EB", "margin": "sm"},
            {"type": "text", "text": f"ชื่อค่าย: {note}",
             "size": "sm", "align": "center", "color": "#374151", "wrap": True},
        ]
                    ...
                    # Body ขาว ตัวหนังสือดำ
                    {
                        ...
                        "contents": [
                            {"type": "text", "text": f"รอบที่ {pair_no}",
                             "weight": "bold", "size": "xxl", "align": "center", "color": "#111827"},
                            {"type": "text", "text": "รอแอดมินออกราคาสักครู่",
                             "size": "sm", "align": "center", "color": "#EF4444"},
                        ] + note_rows
                    }
                ]
            }
                ...
                "contents": [
                    {
                        ...
                        "paddingAll": "14px",
                        "contents": [
                            {"type": "text", "text": "✅ เปิดให้เล่นอีกรอบ!!",
                    ...
                    },
                    {
                        ...
                        "contents": [
                            ...
                            {"type": "separator", "color": "#E5E7EB", "margin": "sm"},
                            {"type": "text", "text": "ยกเลิกบิลพิมพ์ X  •  ดูบัตรสมาชิกพิมพ์ C",
...
def _fmt_amount_range(amount_tuple):
    """แปลง (min, max) หรือ (val, None) เป็น string เช่น '320-350' หรือ '350'"""
    if amount_tuple is None:
        return "-"
    if isinstance(amount_tuple, tuple):
        mn, mx = amount_tuple
        if mx is not None and mx != mn:
            return f"{fmt(mn)}-{fmt(mx)}"
        return fmt(mn)
    # รองรับ int เก่า (backward compat)
...
def flex_open_with_prices(pair_no, camp, hi_amount=None, hi_rate=None, lo_amount=None, lo_rate=None):
    ...
    - hi_amount = tuple (min, max) หรือ None  /  hi_rate = string เช่น "1.7"
    - lo_amount = tuple (min, max) หรือ None  /  lo_rate = string เช่น "1.95"
    รองรับ:  ล350/1.7  /  ล320-350/1.7  /  ราคาเดียว
    """
    rows = [
        {"type": "text", "text": f"🚀 ชื่อค่าย : {camp}",
         "size": "md", "weight": "bold", "wrap": True, "color": "#111827"},
    ...
    rows += [
        ...
        {"type": "text", "text": f"👉 รอบนี้รับ สูง {fmt(SIDE_CAP['HI'])} • ต่ำ {fmt(SIDE_CAP['LO'])}", "size": "sm"},
        {"type": "text", "text": "👉 ออกราคาบั้งไฟหลังปิด ถือว่าจาวทุกกรณี", "size": "sm"},
        {"type": "separator", "margin": "sm"},
        {"type": "text", "text": "📢 ยกเลิกการแทง กด X", "size": "sm"},
        {"type": "text", "text": "📢 ดูยอดบัญชีตัวเอง กด C", "size": "sm"},
    ]
                ...
                "contents": [
                    {
                        ...
                        "paddingAll": "14px",
                        "contents": [
                            {"type": "text", "text": "🔴 หยุดแทง",
                             "weight": "bold", "size": "xl", "align": "center", "color": "#FFFFFF"},
                            {"type": "text", "text": f"ปิดรอบ #{pair_no}",
                             "size": "xs", "align": "center", "color": "#FEE2E2", "margin": "xs"},
                        ]
                    },
                    {
                        "type": "box",
                        ...
                        "contents": [
                            {"type": "text",
                             "text": "ระบบปิดรับบิลแล้ว",
                             "size": "sm", "align": "center", "color": "#374151"},
                            {"type": "text",
                             "text": "กรุณารอสรุปผล / ประกาศราคาถัดไป",
                             "size": "xs", "align": "center", "color": "#9CA3AF", "margin": "xs"},
                        ]
                    }
                ]
            }
                ...
                "contents": [
                    {
                        ...
                        "paddingAll": "14px",
                        "contents": [
                            {"type": "text", "text": "⏸️ หยุดแทงชั่วคราว",
                             "weight": "bold", "size": "lg", "align": "center", "color": "#FFFFFF"},
                            {"type": "text", "text": f"รอบที่ {pair_no}",
                             "size": "xs", "align": "center", "color": "#FEF3C7", "margin": "xs"},
                        ]
                    },
                    {
                        ...
                        "contents": [
                            {"type": "text", "text": f"ค่าย: {camp}",
                             "weight": "bold", "size": "sm", "align": "center", "color": "#111827"},
                            {"type": "text", "text": "รอแอดมินเปิดอีกรอบ",
                             "size": "xs", "align": "center", "color": "#9CA3AF"},
                        ]
                    }
                ]
...
def flex_customer_card(st, user):
    """
    การ์ดสมาชิกแบบกะทัดรัด ตามภาพตัวอย่าง
    แสดง: รูป • ID • ชื่อ • เติมเครดิต • เครดิตคงเหลือ • รายการเล่น
    """
    if not user:
    ...
    side_icon  = "🔵" if side_th == "สูง" else "🔴"

    # แถวบน: รูป + ข้อมูล
    bet_line = f"{side_icon} {side_th} {fmt(stake_used)} บ." if have_bet else "ยังไม่ได้เดิมพัน"
    bet_color = side_color if have_bet else "#9CA3AF"

    return FlexSendMessage(
        alt_text=f"ID: {cid} | {name}",
        contents={
            "type": "bubble",
            "size": "kilo",
            "styles": {"body": {"backgroundColor": "#FFFFFF"}},
            "body": {
                ...
                "contents": [
                        ...
                        "contents": [
                            ...
                            },
                            {
                                ...
                                "contents": [
                                    ...
                                    {
                                        "type": "text",
                                        "text": f"💰 เครดิตคงเหลือ {fmt(credit_total)} บ.",
                                        "size": "xs",
                                        "color": "#6B7280",
                                        "wrap": True
                                    }
                    ...
                    # Admin label (ถ้าเป็น admin)
                    {
                        "type": "text",
                        "text": "(Admin 🏅 แอดมิน 🏅)" if False else "",
                        "size": "xxs",
                        "color": "#9CA3AF",
                        "align": "end"
                    } if False else {"type": "filler"}
                ]
            }
        }
    )
...
def flex_result_preview(code: str, pair_no: int):
    # ---- mapping สี/ไอคอน/คำอธิบาย ตามผล (ระบบใหม่) ----
    # ดึง label จาก RESULT_DEFS ถ้ามี
    rd = RESULT_DEFS.get(code, {})
    label = rd.get("label", "")
    special = rd.get("special", "")
    rate = rd.get("rate", None)

    fee_pct = int(MIDDLE_FEE * 100)  # 3
    if special == "HI_WIN":
        title  = "สูงชนะ"
        accent = "#16A34A"
        icon   = "✅"
        ...
        icon   = "❌"
        desc   = f"จ่าย : {rate}"
    elif special == "DRAW_HI_FEE_LO_LOSE":
        title  = "เสมอสูง"
        accent = "#7C3AED"
        icon   = "⚖️"
        desc   = f"สูงหัก {fee_pct}% / ต่ำเสียหมด"
    elif special == "DRAW_LO_FEE_HI_LOSE":
        title  = "เสมอต่ำ"
        accent = "#7C3AED"
        icon   = "⚖️"
        desc   = f"ต่ำหัก {fee_pct}% / สูงเสียหมด"
    elif special == "DRAW_BOTH_FEE":
        title  = "เสมอ"
        accent = "#7C3AED"
        icon   = "⚖️"
        desc   = f"หัก {fee_pct}% ทั้ง 2 ฝั่ง"
    elif special == "DRAW_BOTH_LOSE":
        # รองรับทั้ง เสมอ2 และ สส
        if code == "สส":
            title = "สส"
        else:
            title = "เสมอ2"
        accent = "#374151"
        icon   = "💥"
        desc   = "ทั้ง 2 ฝั่งเสียหมด"
    elif special == "DRAW_0":
        title  = "จาว (คืนเต็ม)"
        accent = "#22C55E"
        icon   = "🟢"
        desc   = "คืนเงินเต็มจำนวน"
    else:
        title  = f"โค้ด: {code}"
        accent = "#EF4444"
        icon   = "⚠️"
...
def flex_settle(pair_no, rows, footer_text,
    ...
    # --- 1. ส่วนหัวข้อ (Header) ---
    # สร้างข้อความอธิบายผลตาม footer_text
    # footer_text มาจาก settle_by_code เช่น "ผล: สูงชนะ หัก 30% (จ่าย 0.70)"
    result_desc = footer_text.replace("ผล: ", "").strip() if footer_text else ""

    header_contents = [
        {
            "type": "text",
            "text": f"✅ สรุปผลรอบที่ {pair_no}",
            "weight": "bold",
            "align": "center",
            "size": "lg",
            "color": "#FFFFFF"
    ...
    if rows:
        ...
        lines.append({"type": "separator", "margin": "sm", "color": "#E5E7EB"})
        for r in rows:
            ...
            if has_balance:
                ...
                row_cols.append({"type": "text", "text": fmt(bal), "flex": 3, "size": "sm", "align": "end", "color": "#16A34A"})
                    ...
                    # Body ขาว
                    {
                        ...
                        "contents": lines + [
                            ...
                            {"type": "text", "text": footer_text,
                             "align": "end", "size": "xs", "color": "#9CA3AF"}
                        ]
                    }
                ]
            }
    ...
    for idx, item in enumerate(recent):
        ...
        rows.append({
            ...
            "contents": [
                ...
                {
                    "type": "text",
                    "text": f"{style['label']}{style.get('emoji', '')}".strip() or "-",
                    "flex": 3,
                    "size": "sm",
                    "color": style["bg"],
                    "weight": "bold",
...
def settle_by_code(st, code):
    """
    โมเดลเครดิต:
    - ตอนรับบิล: หักเครดิต = amount เก็บไว้ใน escrow
    - ตอนสรุป:
        * HI_WIN/LO_WIN: ฝั่งชนะได้ amount*rate กลับคืน (ต้นทุน+กำไร), ฝั่งแพ้เสียเต็ม
        * DRAW_HI_FEE_LO_LOSE: สูงได้คืนหัก 3%, ต่ำเสียทั้งหมด
        * DRAW_LO_FEE_HI_LOSE: ต่ำได้คืนหัก 3%, สูงเสียทั้งหมด
        * DRAW_BOTH_FEE: ทั้ง 2 ฝั่งได้คืนหัก 3%
        * DRAW_BOTH_LOSE: ทั้ง 2 ฝั่งเสียยอดเดิมพันทั้งหมด
        * DRAW_0: คืนเต็มทั้ง 2 ฝั่ง
    ...
    # DRAW_HI_FEE_LO_LOSE: เสมอสูง - สูงได้คืนหัก 3%, ต่ำเสียทั้งหมด
    if special == "DRAW_HI_FEE_LO_LOSE":
        for b in st["bet_index"].values():
            if b["side"] == "HI":
    ...
    # DRAW_LO_FEE_HI_LOSE: เสมอต่ำ - ต่ำได้คืนหัก 3%, สูงเสียทั้งหมด
    if special == "DRAW_LO_FEE_HI_LOSE":
        for b in st["bet_index"].values():
            if b["side"] == "LO":
                refund = _round_refund(b["amount"] * (1 - MIDDLE_FEE))
                add(b["uid"], b["name"], b["amount"], refund)
    ...
    # DRAW_BOTH_FEE: เสมอ - ทั้ง 2 ฝั่งได้คืนหัก 3%
    if special == "DRAW_BOTH_FEE":
        for b in st["bet_index"].values():
            refund = _round_refund(b["amount"] * (1 - MIDDLE_FEE))
            add(b["uid"], b["name"], b["amount"], refund)
        return list(acc.values()), f"ผล: {d['label']}"

    # DRAW_BOTH_LOSE: เสมอ2 - ทั้ง 2 ฝั่งเสียยอดเดิมพันทั้งหมด
    if special == "DRAW_BOTH_LOSE":
        for b in st["bet_index"].values():
            add(b["uid"], b["name"], b["amount"], 0)
        return list(acc.values()), f"ผล: {d['label']}"
    ...
    for b in st["bet_index"].values():
        add(b["uid"], b["name"], b["amount"], b["amount"])
    return list(acc.values()), f"ผล: {d['label']} (fallback คืนเต็ม)"
...
def on_message(event: MessageEvent):
    ...
    with with_rooms_lock():
        ...
        if t in PLAY_HELP_COMMANDS or t2 in PLAY_HELP_COMMANDS:
            safe_reply(event, TextSendMessage(_get_play_help_text()))
            return
        ...
        if (m_announce or m_announce_single or m_announce_onesided ) and not re.match(r"^\s*o\b", text, re.IGNORECASE):
            ...
            if m_announce:
                ...
                hi_min_  = int(m_announce.group(2))
                hi_max_  = int(m_announce.group(3)) if m_announce.group(3) else None
                hi_rate  = m_announce.group(4)
                lo_min_  = int(m_announce.group(5))
                lo_max_  = int(m_announce.group(6)) if m_announce.group(6) else None
                ...
                am_max  = int(m_announce_single.group(4)) if m_announce_single.group(4) else None
                rate    = m_announce_single.group(5)
                if side_ch == "ล":
                    hi_amount, hi_rate = (am_min, am_max), rate
                    lo_amount, lo_rate = None, None
                else:
        ...
        if re.match(r"^\s*o\b", text, re.IGNORECASE):
            ...
            if m or m_single or m_onesided or m_onesided_lo:
                if m:
                    ...
                    hi_max_  = int(m.group(3)) if m.group(3) else None
                    hi_rate  = m.group(4)
                    lo_min_  = int(m.group(5))
                    lo_max_  = int(m.group(6)) if m.group(6) else None
                    lo_rate  = m.group(7)
                    hi_amount = (hi_min_, hi_max_)
                    lo_amount = (lo_min_, lo_max_)
                elif m_onesided:
                    ...
                    am_min  = int(m_single.group(3))
                    am_max  = int(m_single.group(4)) if m_single.group(4) else None
                    rate    = m_single.group(5)
                    if side_ch == "ล":
                        hi_amount, hi_rate = (am_min, am_max), rate
                        lo_amount, lo_rate = None, None
                    else:
        ...
        if bet:
            with with_users_lock():
                ...
                ok, why, bet_mode = can_bet(st, uid, bet["side"], bet["amount"])
                if not ok:
                ...
                if bet_mode == "ADD":
                    # เพิ่มยอดบิลเดิม
                    existing = st["bet_index"][uid]
                    existing["amount"] += bet["amount"]
                    st["totals"][bet["side"]] += bet["amount"]
                    safe_reply(event, TextSendMessage(
                        f"คุณ {name} ✅ เพิ่มยอด {side_th} +{fmt(bet['amount'])} "
                        f"(รวม {fmt(existing['amount'])}) • ยอดเงินคงเหลือ {fmt(u['credit'])}"
                    ))
                else:
                    # บิลใหม่
